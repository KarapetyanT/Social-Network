...
export const PrivatePublic = () => {
  const { data, refetch } = useHttpQuery<IResponse>("/account/id");
  const [switchAccount] = useHttpMutation<IResponse, { isPrivate: number }>(
    () => refetch()
  );

  const account: IAccount | null = data ? (data.payload as IAccount) : null;
  const isPrivate = account?.isPrivate ?? 0;

  const [localPrivate, setLocalPrivate] = useState(isPrivate);

  const handleSwitch = () => {
    const newPrivacyStatus = localPrivate === 0 ? 1 : 0;
    setLocalPrivate(newPrivacyStatus);
